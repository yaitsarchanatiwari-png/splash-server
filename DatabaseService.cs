...
public class DatabaseService
{
    ...
    private static readonly HttpClient _cloudHttp = new HttpClient();
    ...
    private const string GitHubRepo = "yaitsarchanatiwari-png/splash-server";
    private Timer? _backupDebounceTimer;
    ...
    public DatabaseService(SecurityService security)
    {
        ...
        _backupFilePath = Path.Combine(Path.GetDirectoryName(dbPath)!, "users_backup.json");
    ...
    private void InitializeDatabase()
    {
        ...
        TryAddColumn(conn, "users", "is_admin", "INTEGER NOT NULL DEFAULT 0");
        ...
        using (var fixCmd = conn.CreateCommand())
        {
            fixCmd.CommandText = @"
                ...
                UPDATE users SET device_lock_id = NULL WHERE device_lock_id LIKE 'WEB_%' OR device_lock_id LIKE 'ADMIN%';
                DELETE FROM user_devices WHERE device_id LIKE 'WEB_%' OR device_id LIKE 'ADMIN%';
                UPDATE users SET is_admin = 1 WHERE LOWER(username) IN ('azplayzz', 'admin');
            ";
    ...
    public void CreateUser(UserRecord user, bool triggerCloudBackup = true)
    {
        using var conn = GetConnection();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = @"
            INSERT INTO users (id, username, password_hash, password_salt, created_at_utc, status,
                               ...
                               failed_login_count, lockout_until_utc, is_admin)
            VALUES (@id, @u, @ph, @ps, @cat, @st, @as, @ae, @sa, @satu, @cav, @lip, @lsu, @sec, @dlock, @flc, @lout, @ia)
        ";
        ...
        cmd.Parameters.AddWithValue("@ia", (user.IsAdmin || user.Username.Equals("AzPlayzZ", StringComparison.OrdinalIgnoreCase) || user.Username.Equals("admin", StringComparison.OrdinalIgnoreCase)) ? 1 : 0);
        cmd.ExecuteNonQuery();
        if (triggerCloudBackup)
        {
            ScheduleCloudBackup(immediate: true);
        }
    }

    public void UpdateUser(UserRecord user, bool triggerCloudBackup = true)
    {
        using var conn = GetConnection();

        // Safeguard: Never let a null or WEB_ device lock overwrite an existing valid desktop HWID lock
        if (string.IsNullOrWhiteSpace(user.DeviceLockId) ||
            user.DeviceLockId.StartsWith("WEB_", StringComparison.OrdinalIgnoreCase) ||
            user.DeviceLockId.StartsWith("ADMIN", StringComparison.OrdinalIgnoreCase))
        {
            using var chk = conn.CreateCommand();
            chk.CommandText = "SELECT device_lock_id FROM users WHERE id = @id LIMIT 1";
            chk.Parameters.AddWithValue("@id", user.Id);
            var existingLock = chk.ExecuteScalar()?.ToString();
            if (!string.IsNullOrWhiteSpace(existingLock) &&
                !existingLock.StartsWith("WEB_", StringComparison.OrdinalIgnoreCase) &&
                !existingLock.StartsWith("ADMIN", StringComparison.OrdinalIgnoreCase))
            {
                user.DeviceLockId = existingLock;
            }
        }
    ...
    public bool ValidateOrBindDevice(string userId, string deviceId, string deviceName, out string? error)
    {
        ...
        // Only enforce hardware match if user already has an ACTUAL desktop hardware lock (never web/browser IDs)
        bool hasDesktopLock = !string.IsNullOrWhiteSpace(user.DeviceLockId) &&
                              !user.DeviceLockId.StartsWith("WEB_", StringComparison.OrdinalIgnoreCase) &&
                              !user.DeviceLockId.StartsWith("ADMIN", StringComparison.OrdinalIgnoreCase);
        ...
        {
            // First real desktop hardware binding: lock account to this device
    ...
    public void ResetUserDevice(string userId, string ipAddress = "127.0.0.1")
    {
        ...
        using var cmd = conn.CreateCommand();
        cmd.CommandText = @"
            UPDATE users SET device_lock_id = NULL, security_stamp = @sec WHERE id = @uid;
            DELETE FROM user_devices WHERE user_id = @uid;
        ";
        cmd.Parameters.AddWithValue("@uid", userId);
        cmd.Parameters.AddWithValue("@sec", Guid.NewGuid().ToString("N"));
        cmd.ExecuteNonQuery();
    ...
    private static UserRecord MapUser(SqliteDataReader reader)
    {
        ...
        int adminOrdinal = -1;
        try { adminOrdinal = reader.GetOrdinal("is_admin"); } catch { }
        if (adminOrdinal >= 0 && !reader.IsDBNull(adminOrdinal))
        {
            record.IsAdmin = reader.GetInt32(adminOrdinal) == 1;
        }

        if (record.Username.Equals("AzPlayzZ", StringComparison.OrdinalIgnoreCase) ||
            record.Username.Equals("admin", StringComparison.OrdinalIgnoreCase))
        {
            record.IsAdmin = true;
        }
    ...
    #region Cloud & Local Backup Engine

    public void ScheduleCloudBackup(bool immediate = false)
    {
        SaveBackupLocally();
        if (immediate)
        {
            _ = Task.Run(async () => await SyncBackupToGitHubAsync());
            return;
        }

        _backupDebounceTimer?.Dispose();
        _backupDebounceTimer = new Timer(_ =>
        {
            _ = SyncBackupToGitHubAsync();
        }, null, TimeSpan.FromMilliseconds(500), Timeout.InfiniteTimeSpan);
    }

    private static void WriteFileSafely(string path, string content)
    {
        try
        {
        ...
        {
            Console.WriteLine($"[BackupEngine] WriteFileSafely failed for {path}: {ex.Message}");
            try { File.WriteAllText(path, content, Encoding.UTF8); } catch { }
        }
    }

    public void SaveBackupLocally()
    {
        try
        {
            var users = GetAllUsers();
            ...
            {
                // Never overwrite a non-empty backup with an empty list
                return;
            }

            var items = users.Select(u => new UserBackupItem
            {
                Id = u.Id,
                Username = u.Username,
                ...
                Status = (int)u.Status,
                AccessStartUtc = u.AccessStartUtc,
                AccessEndUtc = u.AccessEndUtc,
                ScheduledAction = u.ScheduledAction,
                ScheduledTimeUtc = u.ScheduledTimeUtc,
                CurrentAppVersion = u.CurrentAppVersion,
                LastIp = u.LastIp,
                LastSeenUtc = u.LastSeenUtc,
                SecurityStamp = u.SecurityStamp,
                DeviceLockId = u.DeviceLockId,
                IsAdmin = u.IsAdmin || u.Username.Equals("AzPlayzZ", StringComparison.OrdinalIgnoreCase) || u.Username.Equals("admin", StringComparison.OrdinalIgnoreCase)
            }).ToList();

            var json = JsonSerializer.Serialize(items, new JsonSerializerOptions { WriteIndented = true });

            WriteFileSafely(_backupFilePath, json);

            var baseBackup = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "data", "users_backup.json");
            if (Path.GetFullPath(baseBackup) != Path.GetFullPath(_backupFilePath))
            {
                WriteFileSafely(baseBackup, json);
            }

            var appDataDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "SplashServer");
            WriteFileSafely(Path.Combine(appDataDir, "users_backup.json"), json);
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[BackupEngine] Failed to save local backup: {ex.Message}");
        }
    }

    public async Task<bool> SyncBackupToGitHubAsync()
    {
        try
        {
            ...
            for (int attempt = 1; attempt <= 3; attempt++)
            {
                ...
                {
                    getReq.Headers.UserAgent.ParseAdd("Splash-Server");
                    getReq.Headers.Authorization = new AuthenticationHeaderValue("Bearer", token);
                    var getResp = await _cloudHttp.SendAsync(getReq);
                    if (getResp.IsSuccessStatusCode)
                    {
                        var bodyStr = await getResp.Content.ReadAsStringAsync();
                ...
                {
                    Console.WriteLine($"[BackupEngine] Conflict pushing backup (attempt {attempt}), retrying with fresh SHA...");
                    await Task.Delay(500);
                }
                else
                {
                    Console.WriteLine($"[BackupEngine] GitHub cloud backup returned status: {putResp.StatusCode}");
                    return false;
                }
            }
            return false;
        ...
        {
            Console.WriteLine($"[BackupEngine] GitHub cloud sync error: {ex.Message}");
            return false;
        }
    }
    ...
    public void RestoreFromBackup()
    {
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(token))
            {
                try
                {
                    ...
                    var task = _cloudHttp.SendAsync(req);
                    task.Wait(8000);
                    if (task.IsCompletedSuccessfully && task.Result.IsSuccessStatusCode)
                    {
                        var bodyStr = task.Result.Content.ReadAsStringAsync().Result;
                        using var doc = JsonDocument.Parse(bodyStr);
                        if (doc.RootElement.TryGetProperty("content", out var contentProp))
                        {
                            var rawBase64 = contentProp.GetString()?.Replace("\n", "").Replace("\r", "");
                            if (!string.IsNullOrEmpty(rawBase64))
                            {
                                json = Encoding.UTF8.GetString(Convert.FromBase64String(rawBase64));
                                Console.WriteLine($"[BackupEngine] Successfully retrieved fresh remote user backup from GitHub ({GitHubRepo}).");
                            }
                        }
                    }
                }
                ...
                {
                    Console.WriteLine($"[BackupEngine] GitHub remote restore check failed: {ghEx.Message}");
                }
            }

            // 2. Fallback: If GitHub API failed or offline, load from local file paths
            if (string.IsNullOrWhiteSpace(json))
            {
                var candidatePaths = new List<string>
                {
                    _backupFilePath,
                    Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "data", "users_backup.json"),
                    Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "users_backup.json"),
                    Path.Combine(Environment.CurrentDirectory, "data", "users_backup.json"),
                    Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "SplashServer", "users_backup.json")
                };
            ...
            var items = JsonSerializer.Deserialize<List<UserBackupItem>>(json);
            ...
            {
                var existing = GetUserByUsername(item.Username);
                var effectiveStatus = (AccessStatus)item.Status;
                if (effectiveStatus == AccessStatus.Approved && item.AccessEndUtc.HasValue && now >= item.AccessEndUtc.Value)
                {
                    effectiveStatus = AccessStatus.Expired;
                }
                ...
                if (existing == null)
                {
                    var user = new UserRecord
                    {
                        ...
                        AccessStartUtc = item.AccessStartUtc,
                        AccessEndUtc = item.AccessEndUtc,
                        ScheduledAction = item.ScheduledAction,
                        ScheduledTimeUtc = item.ScheduledTimeUtc,
                        CurrentAppVersion = item.CurrentAppVersion ?? "1.0.0",
                        LastIp = item.LastIp ?? "127.0.0.1",
                        LastSeenUtc = item.LastSeenUtc,
                        SecurityStamp = string.IsNullOrEmpty(item.SecurityStamp) ? Guid.NewGuid().ToString("N") : item.SecurityStamp,
                        DeviceLockId = item.DeviceLockId,
                        IsAdmin = item.IsAdmin || item.Username.Equals("AzPlayzZ", StringComparison.OrdinalIgnoreCase) || item.Username.Equals("admin", StringComparison.OrdinalIgnoreCase)
                    };
                    CreateUser(user, triggerCloudBackup: false);
                    Console.WriteLine($"[BackupEngine] Restored user '{item.Username}' (Status: {user.Status}, End: {user.AccessEndUtc:u}) from backup.");
                }
                else
                {
                    bool needsExistingUpdate = false;
                    if (existing.Username.Equals("AzPlayzZ", StringComparison.OrdinalIgnoreCase) || existing.Username.Equals("admin", StringComparison.OrdinalIgnoreCase))
                    {
                        if (!existing.IsAdmin)
                        {
                            existing.IsAdmin = true;
                            needsExistingUpdate = true;
                        }
                    }
                    // Update access if backup holds Approved status and has not expired
                    if (effectiveStatus == AccessStatus.Approved)
                    {
                        if (existing.Status != AccessStatus.Approved || existing.AccessEndUtc != item.AccessEndUtc)
                        {
                            existing.Status = AccessStatus.Approved;
                            existing.AccessStartUtc = item.AccessStartUtc;
                            existing.AccessEndUtc = item.AccessEndUtc;
                            needsExistingUpdate = true;
                            Console.WriteLine($"[BackupEngine] Restored Approved status for user '{item.Username}' (End: {existing.AccessEndUtc:u}).");
                        }
                    }
                    if (needsExistingUpdate)
                    {
                        UpdateUser(existing, triggerCloudBackup: false);
                    }
                }
            }
        ...
        {
            Console.WriteLine($"[BackupEngine] Failed to restore from backup: {ex.Message}");
        }
    }
